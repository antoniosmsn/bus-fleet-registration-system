...
interface PasajeroEditFormProps {
  pasajero: Pasajero;
  onSave?: (data: PasajeroFormData) => void;
}

const PasajeroEditForm: React.FC<PasajeroEditFormProps> = ({ pasajero, onSave }) => {
  ...
  const {
    ...
    defaultValues: {
      ...
      empresaSubcontratista: pasajero.empresaSubcontratista || '',
      tipoContrato: pasajero.tipoContrato,
      planilla: pasajero.planilla,
      limiteViajesSemana: pasajero.limiteViajesSemana,
      limiteDiario: pasajero.limiteDiario,
      tipoSubsidio: pasajero.tipoSubsidio,
      subsidioPorcentual: pasajero.subsidioPorcentual,
      subsidioMonto: pasajero.subsidioMonto,
      numeroEmpleadoInterno: pasajero.numeroEmpleadoInterno || '',
      badgeInterno: pasajero.badgeInterno || '',
      tagResidencia: pasajero.tagResidencia || '',
    }
  });
  ...
  const onSubmit = async (data: PasajeroFormData) => {
    try {
      console.log('Datos del formulario editado:', data);
      
      if (onSave) {
        onSave(data);
      }
      
      // Aquí se implementaría la lógica para actualizar el pasajero
      await new Promise(resolve => setTimeout(resolve, 1000)); // Simular API call
      
      toast({
        title: "Pasajero actualizado",
        description: "Los datos del pasajero han sido actualizados exitosamente.",
      });
      ...
      toast({
        title: "Error",
        description: "Hubo un error al actualizar el pasajero.",
        variant: "destructive"
      });
    }
  };
          ...
          <div className="flex justify-between mt-6">
            ...
            <div className="flex space-x-4">
              ...
              <Button type="submit" disabled={isSubmitting}>
                {isSubmitting ? 'Guardando...' : 'Actualizar Pasajero'}
              </Button>
            </div>
          </div>
        </TabsContent>
...
export default PasajeroEditForm;
