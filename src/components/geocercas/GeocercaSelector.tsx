...
interface GeocercaSelectorProps {
  ...
  onSelect: (geocercaId: string | null) => void;
}

const GeocercaSelector = ({ geocercas, selectedGeocercaId, onSelect }: GeocercaSelectorProps) => {
            ...
            filteredGeocercas.map((geocerca) => (
              <div
                ...
                className={`flex items-center px-2 py-2 cursor-pointer rounded ${
                  selectedGeocercaId === geocerca.id 
                    ? 'bg-blue-100' 
                    : 'hover:bg-gray-100'
                }`}
                onClick={() => onSelect(selectedGeocercaId === geocerca.id ? null : geocerca.id)}
              >
