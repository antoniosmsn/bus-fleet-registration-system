...
const CapacidadCumplida: React.FC = () => {
  ...
  const autobusesFiltrados = useMemo(() => {
    return autobuses.filter(autobus => {
      ...
      if (filtros.empresaTransporte && filtros.empresaTransporte !== 'all' && autobus.empresaTransporte !== filtros.empresaTransporte) {
        return false;
      }
      ...
      if (filtros.placa && !autobus.placa.toLowerCase().includes(filtros.placa.toLowerCase())) {
        return false;
      }
      ...
      if (filtros.conductor && !autobus.conductorAsignado.toLowerCase().includes(filtros.conductor.toLowerCase())) {
        return false;
      }
      ...
      if (filtros.horaInicio || filtros.horaFin) {
        const fechaCumplimiento = new Date(autobus.fechaHoraCumplimiento);
        const horaActual = fechaCumplimiento.getHours().toString().padStart(2, '0') + ':' + fechaCumplimiento.getMinutes().toString().padStart(2, '0');
  ...
  const handleUpdateAutobus = (autobusActualizado: AutobusCapacidadCumplida) => {
    setAutobuses(prev => prev.map(autobus => autobus.id === autobusActualizado.id ? autobusActualizado : autobus));
  };
  if (!tienePermisos) {
    return <Layout>
  ...
  return <Layout>
      <div className="space-y-6">
        ...
        {/* Filtros */}
        <FiltrosCapacidadCumplida filtros={filtros} onFiltrosChange={handleFiltrosChange} onLimpiarFiltros={handleLimpiarFiltros} />

        {/* Tabla */}
        <TablaCapacidadCumplida autobuses={autobusesFiltrados} onUpdateAutobus={handleUpdateAutobus} />
      </div>
    </Layout>;
};
